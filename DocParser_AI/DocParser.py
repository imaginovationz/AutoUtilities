...
def get_alignment_name(alignment):
    if alignment == WD_ALIGN_PARAGRAPH.LEFT:
        ...
        return 'distribute'
    #elif alignment == WD_ALIGN_PARAGRAPH.THAI_DISTRIBUTE:
...
def get_effective_font_name(run):
    # 1. Directly set on run
    if run.font and run.font.name:
        return run.font.name
    # 2. Set on run's style
    style = getattr(run, 'style', None)
    while style:
...
def parse_docx(file_path):
    ...
    # start the formatting capture 
        runs_info = []
        for run in para.runs:
            #font_name = get_effective_font_name(run)
            #font_name = get_effective_font_name(doc)
            font_name = get_effective_font_name(run)
            if not font_name:
 ...
 #   for para in doc.paragraphs:
  #      content_type = 'heading' if para.style.name.startswith('Heading') else 'paragraph'
   ...
   # from here i am going to parse the tables only
    ...
    # Find embedded objects (e.g., embedded files)
    embedded_files = []
...
if __name__ == "__main__":
    ...
    #sample_file = "../Files/PS00044.doc"  # Replace with your sample file path
